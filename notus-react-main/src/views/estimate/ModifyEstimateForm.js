...
import ModifyCardEstimatePage1 from "../../components/Cards/ModifyCardEstimatePage1";
import ModifyCardEstimatePage2 from "../../components/Cards/ModifyCardEstimatePage2";
import ModifyCardEstimatePage3 from "../../components/Cards/ModifyCardEstimatePage3";
import { validateForm, updateEstimate } from "../../utils/estimateUtils";

function ModifyEstimateForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateEstimate(formData);
      console.log("Estimate modified successfully:", response);
      alert(`Estimate modified successfully! Estimate Number: ${formData.estimateNo}`);
      setFormData({
        estimateNo: "",
        costCenter: "",
        estimateDt: "",
      ...
      console.error("Failed to modify estimate:", error);
      alert(`Failed to modify estimate: ${error.message}`);
    }
  };

  const tabs = [
    { name: "General Information", content: <ModifyCardEstimatePage1 formData={formData} onChange={handleFormDataChange} errors={errors} onNext={handleNext} /> },
    { name: "Cost & Measurement", content: <ModifyCardEstimatePage2 formData={formData} onChange={handleFormDataChange} errors={errors} onBack={handleBack} onNext={handleNext} /> },
    { name: "Pegging Schedule", content: <ModifyCardEstimatePage3 formData={formData} errors={errors} onBack={handleBack} onSubmit={handleSubmit} /> },
  ];
